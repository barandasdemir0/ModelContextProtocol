...
namespace ModelContextProtocol.WebAPI.Endpoints;
...
public static class AuthorizeEndpoints
{
    ...
    public static void UseAuthorizationEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/oauth/token", async (HttpContext context, TokenService tokenService ) =>
        {
            var form = await context.Request.ReadFormAsync();
            ...
            if (grantType == "authorization_code")
            {
                ...
                var computedChallenge =tokenService.Base64UrlEncode(hash);
                ...
                // JWT üret
                var (accessToken, newRefreshToken) = tokenService.GenerateToken(codeData.Username);
            ...
            if (grantType == "refresh_token")
            {
                ...
                var (accessToken, newRefreshToken) = tokenService.GenerateToken(username);
                return Results.Json(new
                {
                    access_token = accessToken,
                    refresh_token = newRefreshToken,
